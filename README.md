...
- **`main.py`** — เว็บแอป Flask ที่ให้ตรวจจับมือผ่านเบราว์เซอร์แบบเรียลไทม์ โดยใช้ MediaPipe Tasks Vision (JavaScript, `@mediapipe/tasks-vision`) รันที่ฝั่ง client ผ่านกล้องเว็บแคมของเบราว์เซอร์ พร้อมแสดงจำนวนมือที่ตรวจพบ ท่าทาง (มือซ้าย/มือขวา) และดีบักล็อกแบบเรียลไทม์

`hand_demo.py` และ `pose_demo.py` ประมวลผลแบบอะซิงโครนัส (โหมด `LIVE_STREAM`) ทำให้วิดีโอยังคงลื่นไหลในขณะที่ MediaPipe ประมวลผลเฟรมอยู่เบื้องหลัง ส่วน `main.py` ประมวลผลฝั่ง client ด้วย MediaPipe Tasks Vision เวอร์ชัน JavaScript ผ่าน WASM/GPU delegate
...
```
.
├── hand_demo.py                       # สาธิตการตรวจจับ landmark ของมือ (OpenCV, native window)
├── pose_demo.py                       # สาธิตการตรวจจับ landmark ของท่าทางร่างกาย (OpenCV, native window)
├── main.py                            # เว็บแอป Flask สำหรับตรวจจับมือผ่านเบราว์เซอร์
├── templates/
│   └── index.html                     # หน้าเว็บของแอป Flask
├── static/
│   └── script.js                      # โค้ด client-side (MediaPipe Tasks Vision, JS)
├── models/
...
รันเว็บแอป Flask สำหรับตรวจจับมือผ่านเบราว์เซอร์:

```bash
uv run python main.py
```

จากนั้นเปิดเบราว์เซอร์ไปที่ [http://127.0.0.1:5000](http://127.0.0.1:5000) แล้วอนุญาตให้เว็บไซต์เข้าถึงกล้องเมื่อมีการขอสิทธิ์ หน้าเว็บจะแสดงภาพจากกล้องพร้อมจุด landmark ของมือ, จำนวนมือที่ตรวจพบ, ท่าทางของมือซ้าย/ขวา (หิน/กระดาษ/กรรไกร) และดีบักล็อกแบบเรียลไทม์

> **หมายเหตุ:** เว็บแอปนี้รัน Flask development server (`debug=True`) จึงเหมาะสำหรับพัฒนา/ทดสอบเท่านั้น ไม่ควรใช้ในระบบ production
